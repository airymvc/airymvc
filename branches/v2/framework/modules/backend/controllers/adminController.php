...
class adminController extends AppController {
    ...
    public function updateAction() {
        ...
        $this->model->updateItem($field, $where, 'admin');
    ...
    public function addAction() {
        $response_msg = null;
        $this->view->setViewfilepath('admin_addView.php');
        if (!array_key_exists('account_id', $this->params)) {
            //put the varible back into the view
            $this->view->render();
        } else {
            ...
            //get the raw (orginal) mysql results
            $mysql_results = $this->model->addItem($field_value, 'admin');
            if ($mysql_results) {
    ...
    public function showChangeAction() {
        ...
        $number_page = $this->params['number_page'];
        $id = $this->params['id'];
        $mysql_results = $this->model->getItemById($id, 'admin');
     
        $this->switchView ('backend', 'admin_change');
        $this->view->setVariable("mysql_results", $mysql_results);
