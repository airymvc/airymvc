...
<div class="mainbox">
<h3 class="titlebar">%{Search_Result}%</h3>
<div class="mainlist">
<ul>
<?php while ($rows = mysql_fetch_array( $mysql_results , MYSQL_BOTH) ) { 
             $today = $rows['year']."-".$rows['month']."-".$rows['day'];
?>
         <li><span class="datetxt"><?php echo $today; ?></span><a href="<?php echo $httpServerHost;?>/index.php?at=eventDetail&id=<?php echo $rows['id']?>"><?php echo $rows['title']; ?></a><br class="clear" /></li><div class="clear"></div>
<?php } ?>
<br class="clear" />
</ul>
</div>
<div><?php echo $paginator; ?></div>
</div>
<!-- mainbox end -->
</div>
<!-- content end -->
